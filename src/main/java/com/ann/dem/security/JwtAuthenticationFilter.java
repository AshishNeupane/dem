...
import com.ann.dem.dto.LoginViewModel;
...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) {
		...
		UsernamePasswordAuthenticationToken authenticationToken = new UsernamePasswordAuthenticationToken(
				credentials.getUsername().trim(), credentials.getPassword(), new ArrayList<>());
		...
		try {
			...
			try {
				unsuccessfulAuthentication(request, response, null, credentials);
			} catch (IOException e) {
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		if (user != null) {
			...
			// Create JWT Token
			String token = JWT.create().withSubject(principal.getUsername()).withExpiresAt(expireTime)
			...
			finalJson.append("{\"authorization\" :").append(authorizationJson).append(",").append("\"user\" :")
					.append(principal).append(",").append("\"user_information\" :").append(userInformationJsonString)
					.append("}");
			...
			doFilterInternal(request, response, chain, message);
		}
	}
	...
	protected void unsuccessfulAuthentication(HttpServletRequest request, HttpServletResponse response,
		...
		doFilterInternal(request, response, filterChain, null);
	}
	...
	protected void unsuccessfulAuthentication(HttpServletRequest request, HttpServletResponse response,
		...
		doFilterInternal(request, response, filterChain, message);
	}
